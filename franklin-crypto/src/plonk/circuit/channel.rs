...
use crate::plonk::circuit::jive::*;
use crate::jive::JiveEngine;
use crate::rescue::*;
...
use crate::plonk::circuit::affine_point_wrapper::WrappedAffinePoint;
...
#[derive(Clone, Debug)]
pub struct JiveChannelGadget<E: JiveEngine> {
    params: E::Params,
    state: Option<AllocatedNum<E>>
}

impl<E: JiveEngine> ChannelGadget<E> for JiveChannelGadget<E> {
    type Params = E::Params;
    ...
    fn consume<CS: ConstraintSystem<E>>(&mut self, value: AllocatedNum<E>, cs: &mut CS) -> Result<(), SynthesisError> {  
        if let Some(s) = self.state {
            self.state = Some(jive_2_1(cs, &s, &value, &self.params)?);
        } else {
